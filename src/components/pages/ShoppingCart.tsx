...
export const ShoppingCart: React.FC = () => {
  ...
  const columns: GridColDef[] = [
    {
      ...
      renderCell: (params) => (
        <Avatar
          src={
            params.value.startsWith("http") || params.value.length > 40
              ? params.value
              : `images/${params.value}`
          }
    ...
    { field: "price", headerName: "Price", flex: 0.3, type: "number" },
    {
      ...
      getActions: (params) => [
        <GridActionsCellItem
          ...
          icon={<Delete></Delete>}
          onClick={async () =>
            await ordersService.removeShoppingProduct(
              authUser,
              params.id as string
            )
          }
  ...
  async function updateCount(newRow: any): Promise<any> {
    ...
    await ordersService.addShoppingProduct(authUser, rowData.id!, {
      id: rowData.id!,
      count: rowData.count,
    });
    return newRow;
  ...
  function getShoppingProduct(
    ...
    if (!product) {
      ...
      res = {
        ...
        price: +(product.cost * shoppingProduct.count).toFixed(2),
      };
    }
    return res;
  }
  return (
    ...
    >
      ...
      <Typography variant="h6">
        ...
        <img src="images/israeli-shekel-icon.svg" width="3%" />
      </Typography>
      <Snackbar
        open={open}
